...
class FriendsViewModel(val cyclerAPI: CyclerAPI,application: Application) : AndroidViewModel(application){
    ...
    fun findUsers(){
        if(searchInput.value?.length!! < 3)
            return
        cyclerAPI.searchUsers(searchInput.value!!).enqueue(object : Callback<List<UserSearchResult>>{
            override fun onFailure(call: Call<List<UserSearchResult>>, t: Throwable) {
                Toast.makeText(getApplication(),"Something went wrong while loading users!",Toast.LENGTH_SHORT).show()
            }
            ...
            override fun onResponse(
                ...
                if(!response.isSuccessful){
                    ...
                    if(response.body()?.size == 0){
                        Toast.makeText(getApplication(),"No users found!",Toast.LENGTH_SHORT).show()
                    }
