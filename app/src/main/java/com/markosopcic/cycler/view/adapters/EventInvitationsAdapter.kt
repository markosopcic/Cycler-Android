...
class EventInvitationsAdapter(private var requests:ArrayList<EventInvitationResponse>, var onAcceptInvitation : ((String, Boolean, Int) -> Unit)? = null) : RecyclerView.Adapter<EventInvitationsAdapter.EventInvitationViewHolder>(){
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): EventInvitationViewHolder {
        val itemView = LayoutInflater.from(parent.context).inflate(R.layout.event_invite,parent,false)
   ...
   override fun onBindViewHolder(holder: EventInvitationViewHolder, position: Int) {
       ...
       holder.description.text = currentItem.eventDescription
        ...
        holder.acceptButton.setOnClickListener{
             onAcceptInvitation?.invoke(holder.invitationId.text.toString(),true,position)
        }

        holder.denyButton.setOnClickListener{
             onAcceptInvitation?.invoke(holder.invitationId.text.toString(),false,position)
        }
    ...
    fun changeDataset(list: ArrayList<EventInvitationResponse>) {
        requests = list
